  ...
  <div class="settings-menu" id="settingsMenu" hidden>
    ...
    <div class="settings-panel" id="settingsAi" hidden>
      ...
      <div class="settings-body">
        ...
        <p class="settings-hint">点击「添加 API KEY」，选择服务商、模型并填写密钥（已知服务商的 Base URL 会自动填写）。绑定后可用「测试连接」真实调用该服务商验证 Key。密钥仅保存在本机浏览器。绑定多个 API 时，可在列表中把某个设为「⭐ 默认」，阅读器的 AI 语境翻译 / 语法分析会默认调用它。</p>
